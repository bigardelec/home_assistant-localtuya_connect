...
class TuyaCloudApi:
    ...
    def __init__(self, hass, region_code, client_id, secret, user_id):
        ...
        self._access_token = ""
        self.device_list = {}
        # Cached Tuya DP specifications keyed by device_id.
        self.device_specs = {}
    ...
    async def async_get_device_specifications(self, device_id):
        """Obtain DP IDs, Tuya codes, types and ranges for one device.

        This endpoint is intentionally queried lazily during device setup.
        It provides the metadata needed to map local DP IDs to Tuya function
        codes without guessing from brands or current values.
        """
        if device_id in self.device_specs:
            return self.device_specs[device_id], "ok"
        ...
        if not r_json.get("success"):
            return None, f"Error {r_json.get('code')}: {r_json.get('msg')}"

        result = r_json.get("result") or {}
        self.device_specs[device_id] = result

        # Log only functional metadata. Never log local_key, credentials, UUID,
        # tokens or the complete cloud device record.
        safe_spec = {
        ...
        _LOGGER.debug(
            "Cloud DP specifications for device %s: %s",
            device_id,
