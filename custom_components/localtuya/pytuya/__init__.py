...
try:
    TuyaHeader = namedtuple(
        "TuyaHeader", "prefix seqno cmd length total_length", defaults=(None,)
    )
except TypeError:
...
REQ_DEVINFO = 0x25  # 37 # Tuya 3.5 discovery solicitation request (UDP/7000)
...
MESSAGE_HEADER_6699_FMT = ">IHIII"  # prefix, unknown, seqno, cmd, length
...
PREFIX_6699_BIN = b"\x00\x00f\x99"
SUFFIX_6699_VALUE = 0x00009966
SUFFIX_6699_BIN = b"\x00\x00\x99f"
...
def pack_message(msg, hmac_key=None):
    ...
    if msg.prefix == PREFIX_VALUE:
        ...
        header_fmt = MESSAGE_HEADER_6699_FMT
    ...
    if msg.prefix == PREFIX_6699_VALUE:
        ...
        raw = msg.payload
        if isinstance(msg.retcode, int):
            raw = struct.pack(MESSAGE_RETCODE_FMT, msg.retcode) + raw
        buffer += (
            cipher.encrypt(
                raw,
                use_base64=False,
...
def unpack_message(data, hmac_key=None, header=None, no_retcode=False, logger=None):
    ...
    if header.prefix == PREFIX_VALUE:
        ...
        header_len = struct.calcsize(MESSAGE_HEADER_6699_FMT)
        end_fmt = MESSAGE_END_FMT_6699
        retcode_len = 0
        msg_len = header_len + header.length + len(SUFFIX_6699_BIN)
    else:
        raise ValueError(
    ...
    expected_suffix = (
        SUFFIX_6699_VALUE if header.prefix == PREFIX_6699_VALUE else SUFFIX_VALUE
    )
    if suffix != expected_suffix:
        logger.debug("Suffix prefix wrong! %08X != %08X", suffix, expected_suffix)

    if have_crc is not None and crc != have_crc:
        if hmac_key:
            logger.debug(
    ...
    return TuyaMessage(
        ...
        crc == have_crc,
        header.prefix,
        iv if header.prefix == PREFIX_6699_VALUE else None,
    )
...
def parse_header(data):
    ...
    if data[:4] == PREFIX_6699_BIN:
        ...
        raise DecodeError("Header prefix wrong! %08X" % struct.unpack(">I", data[:4])[0])
    ...
    unpacked = struct.unpack(header_fmt, data[:header_len])
    prefix = unpacked[0]

    if prefix == PREFIX_VALUE:
        prefix, seqno, cmd, payload_len = unpacked
        total_length = struct.calcsize(MESSAGE_HEADER_FMT) + payload_len
    elif prefix == PREFIX_6699_VALUE:
        prefix, _unknown, seqno, cmd, payload_len = unpacked
        total_length = header_len + payload_len + len(SUFFIX_6699_BIN)
    else:
        raise DecodeError("Header prefix wrong! %08X" % prefix)

    # sanity check. currently the max payload length is somewhere around 300 bytes
    if payload_len > 2000:
        raise DecodeError(
            "Header claims the packet size is over 2000 bytes! It is most likely corrupt. Claimed size: %d bytes"
            % payload_len
        )
...
class MessageDispatcher(ContextualLogger):
    ...
    def __init__(self, dev_id, listener, protocol_version, local_key, enable_debug):
        ...
        self.buffer = b""
        self.listeners = {}
        # CONTROL_NEW acknowledgements on some Tuya 3.4/3.5 devices do not
        # reuse the request sequence number. Track pending controls separately
        # and match their ACKs in send order.
    ...
    def abort(self):
        ...
        for future in self.control_listeners:
            if not future.done():
                # Wake pending CONTROL_NEW exchanges cleanly.  Returning None
                # lets exchange() treat a disconnect like the legacy seqno waiters.
                future.set_result(None)
        self.control_listeners.clear()

    def prepare_control_waiter(self):
        """Register a pending CONTROL_NEW request."""
        future = asyncio.get_running_loop().create_future()
        self.control_listeners.append(future)
        return future

    def cancel_control_waiter(self, future):
        """Remove a pending CONTROL_NEW request."""
        if future in self.control_listeners:
            self.control_listeners.remove(future)
        if not future.done():
            future.cancel()

    async def wait_for_control(self, future, timeout=5):
        """Wait for the next CONTROL_NEW acknowledgement."""
        try:
            return await asyncio.wait_for(asyncio.shield(future), timeout=timeout)
        except asyncio.TimeoutError:
            self.cancel_control_waiter(future)
            self.debug("CONTROL_NEW timed out waiting for ACK")
            raise
    ...
    def add_data(self, data):
        ...
        while self.buffer:
            prefix_positions = [
                pos
                for pos in (
                    self.buffer.find(PREFIX_BIN),
                    self.buffer.find(PREFIX_6699_BIN),
                )
                if pos >= 0
            ]
            if not prefix_positions:
                self.debug("Invalid prefix: %r", self.buffer)
                self.buffer = b""
                break

            prefix_index = min(prefix_positions)
            if prefix_index > 0:
                self.debug("Prefix is not at start; discarding %d bytes", prefix_index)
                self.buffer = self.buffer[prefix_index:]

            header_len = (
                struct.calcsize(MESSAGE_HEADER_6699_FMT)
                if self.buffer.startswith(PREFIX_6699_BIN)
                else struct.calcsize(MESSAGE_RECV_HEADER_FMT)
            )
            ...
            msg = unpack_message(
                self.buffer, header=header, hmac_key=hmac_key,
                no_retcode=None, logger=self
            )
    ...
    def _dispatch(self, msg):
        ...
        # STATUS packets on 3.4/3.5 are asynchronous state reports. Do not let
        # their sequence number accidentally satisfy an unrelated request.
        if self.version >= 3.4 and msg.cmd == STATUS:
        ...
        # CONTROL_NEW ACK sequence numbers are not reliable on every 3.4/3.5
        # device. Match ACKs to pending controls in TCP/send order instead.
        if (
            self.version >= 3.4
            and msg.cmd == CONTROL_NEW
            and self.control_listeners
        ):
            future = self.control_listeners.pop(0)
            if not future.done():
                future.set_result(msg)
            self.debug(
                "Matched CONTROL_NEW response using FIFO control queue "
                "(received seqno %d)",
                msg.seqno,
            )
            return
        ...
        if msg.seqno in self.listeners:
            ...
            if msg.cmd == CONTROL_NEW:
                self.debug(
                    "Got unsolicited ACK message for command %d: "
                    "no pending CONTROL_NEW request",
                    msg.cmd,
                )
            else:
                self.debug(
...
class TuyaProtocol(asyncio.Protocol, ContextualLogger):
    ...
    def connection_lost(self, exc):
        ...
        if self.dispatcher is not None:
            self.dispatcher.local_key = self.local_key
            # Wake all pending exchanges immediately instead of leaving them
            # blocked until their individual timeout expires.
    ...
    async def close(self):
        ...
        if self.heartbeater is not None:
            ...
            self.heartbeater = None
        if self.dispatcher is not None:
            # Keep the dispatcher object alive until this protocol instance is
            # garbage-collected. Pending coroutines may still resume after
            # abort(); replacing it with None creates a race in exchange().
            self.dispatcher.abort()
    ...
    async def exchange(self, command, dps=None):
        ...
        # The connection may have been closed by the heartbeat or by another
        # exchange while this coroutine was preparing its payload.
        if self.transport is None or self.dispatcher is None:
            self.debug("Exchange aborted: device is no longer connected")
            return None

        control_waiter = None
        if self.version >= 3.4 and real_cmd == CONTROL_NEW:
            control_waiter = self.dispatcher.prepare_control_waiter()
        ...
        try:
            ...
            if control_waiter is not None:
                self.dispatcher.cancel_control_waiter(control_waiter)
            await self.close()
            return None

        if control_waiter is not None:
            msg = await self.dispatcher.wait_for_control(control_waiter)
        else:
            msg = await self.dispatcher.wait_for(seqno, payload.cmd)
        if msg is None:
        ...
        if real_cmd in [HEART_BEAT, CONTROL, CONTROL_NEW] and len(msg.payload) <= 4:
    ...
    def _decode_payload(self, payload):
        ...
        if (
            ...
            # strip the leftover 4-byte retcode so version-header
            # detection below sees correctly-aligned data
            payload = payload[4:]
    ...
    async def _negotiate_session_key(self):
        ...
        if self.version == 3.4:
            try:
                ...
                return False
        elif len(payload) == 52 and payload[:4] == b"\x00\x00\x00\x00":
            # some 3.5 devices prepend a 4-byte retcode to this response
            payload = payload[4:]
    ...
    def _encode_message(self, msg):
        ...
        if self.version >= 3.4:
            ...
            if self.version >= 3.5:
                ...
                return pack_message(msg, hmac_key=hmac_key)
