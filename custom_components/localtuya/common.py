...
class TuyaDevice(pytuya.TuyaListener, pytuya.ContextualLogger):
    ...
    def __init__(self, hass, config_entry, dev_id):
        ...
        self._entities = []
        # Track expected offline periods (for example a lamp switched off at the wall)
        # so reconnect attempts do not flood Home Assistant logs.
    ...
    async def _make_connection(self):
        ...
        try:
            ...
            # A device can legitimately be unreachable when it is physically
            # powered off. Report the transition once, then keep subsequent
            # reconnect failures at DEBUG level only.
            if not self._offline_reported:
                self.info(
                    "Device is offline/unreachable; automatic reconnection will continue"
                )
                self._offline_reported = True
            else:
                self.debug("Reconnect attempt failed: %s", ex)
            if self._interface is not None:
                await self._interface.close()
                self._interface = None
        except Exception as ex:  # pylint: disable=broad-except
            # Protocol, authentication or decoding failures are not normal
            # offline conditions and should remain visible to the user.
            self.warning("Connection failed: %s", ex)
            if self._interface is not None:
                await self._interface.close()
                self._interface = None
        ...
        if self._interface is not None:
            try:
                try:
                    ...
                    self.warning(
                        "Initial state update failed (%s), trying key update", ex
                    )
        ...
        if self._interface is not None:
            ...
            if self._offline_reported:
                self.info("Device is back online and connected")
                self._offline_reported = False
            else:
                self.info(
    ...
    def disconnected(self):
        ...
        if not self._offline_reported:
            self.info(
                "Device went offline; waiting for discovery/reconnection"
            )
            self._offline_reported = True
        else:
            self.debug("Device still offline; waiting for discovery/reconnection")
...
class LocalTuyaEntity(RestoreEntity, pytuya.ContextualLogger):
    ...
    def has_entity_name(self):
        """Return whether Home Assistant should compose the translated entity name."""
        return bool(self._config.get(CONF_TRANSLATION_KEY))

    @property
    def translation_key(self):
        """Return the translation key for auto-configured entities."""
        return self._config.get(CONF_TRANSLATION_KEY)

    @property
    def entity_registry_enabled_default(self):
        """Return whether a newly discovered entity is enabled by default."""
        return self._config.get(CONF_ENTITY_REGISTRY_ENABLED_DEFAULT, True)

    @property
    def name(self):
        """Get name of Tuya entity.

        Auto-configured entities use Home Assistant's native translated-name
        resolver.  LocalTuya historically overrides ``name`` for legacy
        entities; returning ``None`` here would incorrectly mark every
        translated entity as the device's main feature and make them all
        display with the device name only.
        """
        if self.has_entity_name:
            # Delegate to Entity.name so translation_key is resolved from the
            # integration's entity translations (and state translations keep
            # using the same key).
            return Entity.name.__get__(self, type(self))
