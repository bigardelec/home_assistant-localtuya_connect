...
async def async_setup(hass: HomeAssistant, config: dict):
    ...
    def _device_discovered(device):
        ...
        if not device:
            # Discovery packets can race with config-entry reload/unload. This is
            # benign and should not create a warning (or expose the full device id).
            _LOGGER.debug(
                "Discovery received for a device not present in the runtime registry; ignoring"
            )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if entry.data.get(CONF_AUTH_TYPE) == AUTH_TYPE_DEVICE_SHARING:
        tuya_api = TuyaSharingApi(
            hass,
        ...
        if res != "ok":
            _LOGGER.error("Device Sharing connection failed: %s", res)
        else:
            _LOGGER.info(
                "Device Sharing connection succeeded with %d devices.",
                len(tuya_api.device_list),
            )
    else:
        # Backwards compatibility for existing LocalTuya entries configured with
