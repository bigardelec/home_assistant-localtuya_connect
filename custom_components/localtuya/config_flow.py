...
from .auto_config import analyze_auto_entities
from .sharing_api import TUYA_CLIENT_ID, TUYA_SCHEMA, TuyaSharingApi
...
CONFIGURE_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_ACTION, default=CONF_ADD_DEVICE): selector.SelectSelector(
            selector.SelectSelectorConfig(
                options=CONF_ACTIONS,
                translation_key="localtuya_action",
...
QR_USER_SCHEMA = vol.Schema({vol.Required(CONF_USER_CODE): cv.string})
...
DEVICE_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(CONF_PROTOCOL_VERSION, default="auto"): selector.SelectSelector(
            selector.SelectSelectorConfig(
                options=["auto", "3.1", "3.2", "3.3", "3.4", "3.5"],
                translation_key="protocol_version",
            )
        ),
        vol.Required(CONF_ENABLE_DEBUG, default=False): bool,
        vol.Required(CONF_AUTO_CONFIG, default=True): bool,
...
PICK_ENTITY_SCHEMA = vol.Schema(
    {
        vol.Required(PLATFORM_TO_ADD, default="switch"): selector.SelectSelector(
            selector.SelectSelectorConfig(
                options=PLATFORMS,
                translation_key="entity_platform",
            )
        )
    }
)
...
def devices_schema(discovered_devices, cloud_devices_list, add_custom_device=True, excluded_device_ids=None):
    """Create schema for devices step.

    LAN-discovered devices keep their discovered IP address.  Devices known only
    through Tuya Device Sharing are also offered and are marked as requiring a
    manual IP address.  This is useful on networks where UDP discovery cannot
    reach Home Assistant (for example Docker Desktop on Windows).
    """
    devices = {}
    excluded_device_ids = set(excluded_device_ids or ())
    for dev_id, dev_host in discovered_devices.items():
        if dev_id in excluded_device_ids:
            continue
        dev_name = dev_id
        if dev_id in cloud_devices_list:
            dev_name = cloud_devices_list[dev_id].get(CONF_NAME) or dev_id
        devices[dev_id] = f"{dev_name} ({dev_host})"

    for dev_id, cloud_dev in cloud_devices_list.items():
        if dev_id in excluded_device_ids or dev_id in devices:
            continue
        dev_name = cloud_dev.get(CONF_NAME) or dev_id
        # Keep dynamic option labels language-neutral. The following form makes
        # it clear that an IP address must be entered when LAN discovery did not
        # provide one.
    ...
    return vol.Schema(
        {
            ...
            vol.Required(CONF_HOST): cv.string,
            vol.Required(CONF_LOCAL_KEY): cv.string,
            vol.Required(CONF_PROTOCOL_VERSION, default="3.3"): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=["auto", "3.1", "3.2", "3.3", "3.4", "3.5"],
                    translation_key="protocol_version",
                )
...
async def validate_input(hass: core.HomeAssistant, data):
    ...
    When protocol version is set to ``auto`` (used for QR-only devices whose
    LAN discovery is unavailable), probe supported Tuya protocol versions and
    keep the first one that yields a valid DPS list.  The detected version is
    written back to ``data`` so it is persisted in the device configuration.
    """
    if data.get(CONF_PROTOCOL_VERSION) == "auto":
        last_error = None
        for version in ("3.5", "3.4", "3.3", "3.2", "3.1"):
            trial = data.copy()
            trial[CONF_PROTOCOL_VERSION] = version
            _LOGGER.debug(
                "Auto-detecting Tuya protocol for %s: trying %s",
                data.get(CONF_DEVICE_ID),
                version,
            )
            try:
                dps = await validate_input(hass, trial)
            except (CannotConnect, InvalidAuth, EmptyDpsList) as ex:
                last_error = ex
                _LOGGER.debug(
                    "Tuya protocol %s did not validate for %s: %s",
                    version,
                    data.get(CONF_DEVICE_ID),
                    ex,
                )
                ...
                last_error = ex
                _LOGGER.debug(
                    "Tuya protocol %s probe failed for %s: %s",
                    version,
                    data.get(CONF_DEVICE_ID),
                    ex,
                )
            ...
            _LOGGER.info(
                "Auto-detected Tuya protocol %s for device %s",
                version,
                data.get(CONF_DEVICE_ID),
            )
            return dps
...
class LocaltuyaConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self):
        ...
        self._qr_token = None
        self._qr_user_code = None

    async def async_step_user(self, user_input=None):
        """Ask for Smart Life / Tuya Smart user code and mint a QR token."""
        errors = {}
        placeholders = {}
        if user_input is not None:
            ...
            try:
                ...
                if response.get("success", False):
                    ...
                    self._qr_token = response["result"]["qrcode"]
                ...
                _LOGGER.exception("Could not start Device Sharing QR login: %s", ex)
                errors["base"] = "qr_login_error"
                placeholders = {"msg": str(ex), "code": "connection"}

        return self.async_show_form(
            step_id="user",
            data_schema=QR_USER_SCHEMA,
            errors=errors,
            description_placeholders=placeholders,
        )

    async def async_step_scan(self, user_input=None):
        """Display QR code and complete Device Sharing login."""
        if self._qr_token is None or self._qr_user_code is None:
        ...
        qr_schema = vol.Schema(
            {
                vol.Optional("QR"): selector.QrCodeSelector(
                    config=selector.QrCodeSelectorConfig(
                        data=f"smartlife--qrLogin?token={self._qr_token}",
                        scale=5,
        ...
        try:
            ok, info = await self.hass.async_add_executor_job(
            ...
            _LOGGER.exception("Device Sharing QR confirmation failed: %s", ex)
            return self.async_show_form(
                step_id="scan",
                data_schema=qr_schema,
                errors={"base": "qr_login_error"},
                description_placeholders={"msg": str(ex), "code": "connection"},
            )

        if not ok:
            # The token may have expired. Mint a replacement and show it immediately.
            try:
                response = await self.hass.async_add_executor_job(
                    self._login_control.qr_code,
                    TUYA_CLIENT_ID,
            ...
            return self.async_show_form(
                ...
                description_placeholders={
                    "msg": str(info.get("msg", "Login not confirmed or QR expired")) if isinstance(info, dict) else "Login not confirmed or QR expired",
                    "code": str(info.get("code", "0")) if isinstance(info, dict) else "0",
                },
            )
        ...
        entry_data = {
            CONF_AUTH_TYPE: AUTH_TYPE_DEVICE_SHARING,
            CONF_USER_CODE: self._qr_user_code,
            CONF_TOKEN_INFO: token_info,
            CONF_TERMINAL_ID: info.get(CONF_TERMINAL_ID),
            CONF_ENDPOINT: info.get(CONF_ENDPOINT) or info.get("end_point"),
            CONF_USERNAME: info.get("username") or DOMAIN,
            CONF_NO_CLOUD: False,
            CONF_REGION: "eu",
            CONF_CLIENT_ID: "",
            CONF_CLIENT_SECRET: "",
            CONF_USER_ID: info.get("uid") or self._qr_user_code,
            CONF_DEVICES: {},
        }

        # Validate that the resulting session can actually retrieve the account devices.
        sharing_api = TuyaSharingApi(
            self.hass,
        ...
        result = await sharing_api.async_get_devices_list()
        if result != "ok":
            return self.async_show_form(
                step_id="scan",
                data_schema=qr_schema,
                errors={"base": "device_list_failed"},
                description_placeholders={"msg": result, "code": "device_list"},
            )

        _LOGGER.info(
            "Device Sharing QR login succeeded; %d devices available.",
            len(sharing_api.device_list),
        )
        await self.async_set_unique_id(str(info.get("uid") or self._qr_user_code))
    ...
    async def async_step_add_device(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=devices_schema(
                ...
                True,
                self.config_entry.data[CONF_DEVICES].keys(),
            ),
            errors=errors,
        )
    ...
    async def async_step_configure_device(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                # validate_input may replace the manual-IP ``auto`` selection
                # with the protocol version that actually responded.
                self.device_data[CONF_PROTOCOL_VERSION] = user_input[
                    CONF_PROTOCOL_VERSION
                ]

                if user_input.get(CONF_AUTO_CONFIG, True) and dev_id is not None:
                    cloud_api = self.hass.data[DOMAIN][DATA_CLOUD]
                    try:
                        spec, spec_result = await cloud_api.async_get_device_specifications(dev_id)
                    except Exception as ex:
                        spec, spec_result = None, str(ex)
                        _LOGGER.debug(
                            "Auto-config metadata lookup failed for %s: %s", dev_id, ex
                        )

                    if spec_result == "ok" and spec:
                        available_dp_ids = {
                            int(item.split(" ", 1)[0]) for item in self.dps_strings
                        }
                        auto_report = analyze_auto_entities(
                            spec,
                            available_dp_ids,
                            self.device_data[CONF_FRIENDLY_NAME],
                        )
                        auto_entities = auto_report.entities
                        _LOGGER.info(
                            "Auto-config analysis for %s: created=%d mapped=%s ignored=%s missing_local_dp=%s rejected_type=%s",
                            dev_id,
                            len(auto_entities),
                            auto_report.mapped_codes,
                            auto_report.ignored_codes,
                            auto_report.missing_local_dps,
                            auto_report.rejected_codes,
                        )
                        if auto_entities:
                            self.entities = auto_entities
                            self.device_data[CONF_DPS_STRINGS] = self.dps_strings
                            self.device_data[CONF_ENTITIES] = self.entities
                            new_data = self.config_entry.data.copy()
                            new_data[ATTR_UPDATED_AT] = str(int(time.time() * 1000))
                            new_data[CONF_DEVICES].update(
                                {self.device_data[CONF_DEVICE_ID]: self.device_data}
                            )
                            self.hass.config_entries.async_update_entry(
                                self.config_entry, data=new_data
                            )
                            _LOGGER.info(
                                "Auto-configured %d entities for device %s: %s",
                                len(self.entities),
                                dev_id,
                                [
                                    f"{entity[CONF_PLATFORM]}:{entity[CONF_ID]}"
                    ...
                    _LOGGER.info(
                        "No safe automatic entity mapping found for %s; falling back to manual entity setup.",
                        dev_id,
                    )
                ...
                # A powered-off or temporarily unreachable LAN device is an
                # expected connectivity condition, not an unexpected exception.
                _LOGGER.debug("Device is currently unreachable during setup: %s", ex)
                errors["base"] = "cannot_connect"
            except Exception as ex:
        ...
        if self.editing_device:
            ...
            if dev_id in cloud_devs:
                ...
                if defaults[CONF_LOCAL_KEY] != cloud_local_key:
                    _LOGGER.info(
                        "A new local_key was retrieved; replacing the stored key."
                    )
            ...
            if dev_id is not None:
                # Always use the selected Tuya device id.  LAN discovery may be
                # unavailable (notably with Docker Desktop), in which case the
                # user only needs to enter the device's LAN IP manually.
                defaults[CONF_DEVICE_ID] = dev_id

                device = self.discovered_devices.get(dev_id)
                if device is not None:
                    defaults[CONF_HOST] = device.get("ip") or ""
                    defaults[CONF_DEVICE_ID] = device.get("gwId") or dev_id
                    defaults[CONF_PROTOCOL_VERSION] = device.get("version") or "auto"

                cloud_api = self.hass.data[DOMAIN][DATA_CLOUD]
                cloud_devs = cloud_api.device_list
                if dev_id in cloud_devs:
                    cloud_dev = cloud_devs[dev_id]
                    defaults[CONF_LOCAL_KEY] = cloud_dev.get(CONF_LOCAL_KEY) or ""
                    defaults[CONF_FRIENDLY_NAME] = cloud_dev.get(CONF_NAME) or dev_id

                    # Device Sharing already exposes specification-like metadata,
                    # including local_strategy (numeric local DP -> Tuya code).
                    try:
                        _spec, spec_result = (
                            await cloud_api.async_get_device_specifications(dev_id)
                        )
                        if spec_result != "ok":
                            _LOGGER.debug(
                                "Cloud DP specifications unavailable for device %s: %s",
                                dev_id,
                                spec_result,
                            )
                    except Exception as ex:
                        # Cloud metadata must never block manual LocalTuya setup.
                        _LOGGER.debug(
                            "Could not load cloud DP specifications for device %s: %s",
                            dev_id,
                            ex,
                        )
